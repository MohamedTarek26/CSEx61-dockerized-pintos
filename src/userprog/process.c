...
static bool load(struct arguments *arg, void (**eip)(void), void **esp);
...
static bool setup_stack(void **esp, const struct arguments *arg);
...
bool load(struct arguments *arg, void (**eip)(void), void **esp)
{
  char *file_name = arg->v[0];
  struct thread *t = thread_current();
...
void add_string(void **esp, const char *s)
{
...
/* Create a minimal stack by mapping a zeroed page at the top of
   user virtual memory. */
static bool
setup_stack(void **esp, const struct arguments *arg)
{
  uint8_t *kpage;
  bool success = false;
  ...
  if (kpage != NULL)
  {
    ...
    if (success)
    {
      ...
      *(char *)*esp = 0;
      ...
      // *esp -=  sizeof(char **);
      // *(char *)*esp = argv_add;
      add_char_pointer_pointer(esp, *esp);
      // push argc
      *esp -= sizeof(int);
      *((int *)*esp) = arg->c;
